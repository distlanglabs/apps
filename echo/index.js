    ...
    POST: {
      "/echo/config": async ({ req, state, params }) => {
        try {
          ...
          return helperErrorResponse("configure echo", error);
        }
      },
    },
    GET: {
      "/echo/:text": async ({ req, state, params }) => {
        try {
          ...
          return helperErrorResponse("echo text", error);
        }
      },
    },
  },
...
function helperErrorResponse(action, error) {
  const message = error instanceof Error ? error.message : String(error);
  return Response.json(
    {
      ok: false,
      error: "helper_failure",
      action,
      message,
    },
    { status: 500 },
